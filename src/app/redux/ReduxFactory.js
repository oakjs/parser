...
////////////////////
  ...
  addAction(actionParams) {
    ...
    this.assert(handler || ACTION || actionCreator,
    ...
    const actionHandler = (state, action) => {
      ...
      callPromise
        ...
        .then(result => {
          ...
          this.info(`Completed asyncAction '${actionName}' with result:`, typeof result === "string" ? `${result.substr(0, 200)}...}` : result);
          DONE.result = result;
          dispatch(DONE);
        })
...
////////////////////
  ...
  getActionCreator(actionName, ACTION, getParams, once, addPromise) {
    ...
    return (...args) => {
      ...
      if (params === IGNORE_ACTION)
        return IGNORE_ACTION;

      if (once) {
        const callHashKey = once.call(this, params);
        if (this.hasCalled(actionName, callHashKey)) {
          this.debug(`getActionCreator(${actionName}): cancelling for hash key ${this._getCallSignature(actionName, callHashKey)}`);
          return IGNORE_ACTION;
        }
  ...
  static createStore({
    ...
    if (useDevtools
     ...
     && window.__REDUX_DEVTOOLS_EXTENSION__
      ...
      appliedMiddleware = compose(appliedMiddleware, window.__REDUX_DEVTOOLS_EXTENSION__());
    }
...
////////////////////
  ...
  setUntrackedState(key, value) {
    if (!this.UNTRACKED_STATE)
      this.UNTRACKED_STATE = {};

    if (value === undefined)
      delete this.UNTRACKED_STATE[key];
    else
      this.UNTRACKED_STATE[key] = value;
