...
SpellFileLocation.useRegistry = false

describe("SpellFileLocation", () => {
  describe("creation", () => {
    test("Can create via `new`", () => {
      const location = new SpellFileLocation("@user:projects:PROJECT")
      expect(location).toBeInstanceOf(SpellFileLocation)
    })
    test("if `useRegistry` is set, Creating via `new SpellFileLocation()` always returns the same object", () => {
      SpellFileLocation.useRegistry = true
      const location1 = new SpellFileLocation("@user:projects:PROJECT")
      const location2 = new SpellFileLocation("@user:projects:PROJECT")
      SpellFileLocation.useRegistry = false
  ...
  describe("valid project paths", () => {
    test("valid project path", () => {
      const path = new SpellFileLocation("@user:projects:PROJECT")
    ...
    test("valid library path", () => {
      const path = new SpellFileLocation("@system:examples:PROJECT")
  ...
  describe("invalid project paths", () => {
    test("invalid user", () => {
      const path = new SpellFileLocation("@INVALID_USER/projects/PROJECT")
    ...
    test("invalid domain", () => {
      const path = new SpellFileLocation("@user:INVALID_DOMAIN:PROJECT")
      expect(path.isValidPath).toBe(false)
      expect(path.isProjectPath).toBe(false)
      expect(path.isSystemProject).toBe(false)
      expect(path.isUserProject).toBe(false)
    ...
    test("no project name", () => {
      const path = new SpellFileLocation("@user:projects")
      expect(path.isValidPath).toBe(false)
      expect(path.isSystemProject).toBe(false)
      expect(path.isUserProject).toBe(false)
      expect(path.isProjectPath).toBe(false)
  ...
  describe("valid file paths", () => {
    test("valid file path with no folder and normal extension", () => {
      const path = new SpellFileLocation("@user:projects:PROJECT/FILE.EXTENSION")
    ...
    test("valid file path with single folder and normal extension", () => {
      const path = new SpellFileLocation("@user:projects:PROJECT/FOLDER/FILE.EXTENSION")
      expect(path.projectId).toBe("@user:projects:PROJECT")
      expect(path.owner).toBe("@user")
      expect(path.domain).toBe("projects")
      expect(path.projectName).toBe("PROJECT")
    ...
    test("valid file path with single folder and normal extension", () => {
      const path = new SpellFileLocation("@user:projects:PROJECT/FOLDER1/FOLDER2/FILE.EXTENSION")
      expect(path.projectId).toBe("@user:projects:PROJECT")
      expect(path.owner).toBe("@user")
      expect(path.domain).toBe("projects")
      expect(path.projectName).toBe("PROJECT")
    ...
    test("valid file path with no extension", () => {
      const path = new SpellFileLocation("@user:projects:PROJECT/FILE")
    ...
    test("valid file path with multi-part extension", () => {
      const path = new SpellFileLocation("@user:projects:PROJECT/FILE.EXT1.EXT2")
