...
//
//  Parse a rule with `rule.parse(scope, tokens, start, end, rules)`, this will either:
...
export default class Rule {
  ...
  test(scope, tokens, start, end, rules, testLocation) {}
...
Rule.TokenType = class tokenType extends Rule {
  test(scope, tokens, start = 0, end = tokens.length, rules, testLocation = this.testLocation) {
  ...
  parse(scope, tokens, start = 0, end, rules) {
...
Rule.Literal = class literal extends Rule {
  ...
  parse(scope, tokens, start = 0, end, rules) {
    if (!this.test(scope, tokens, start, end, rules, TestLocation.AT_START)) return undefined;
...
Rule.Literals = class literals extends Rule {
  ...
  parse(scope, tokens, start = 0, end = tokens.length, rules) {
...
Rule.Subrule = class subrule extends Rule {
  ...
  test(scope, tokens, start, end, rules = scope.parser.rules, testLocation = this.testLocation) {
    const rule = Rule.getRuleOrDie(rules, this.subrule);
    return rule.test(scope, tokens, start, end, rules, testLocation);
  }

  parse(scope, tokens, start = 0, end = tokens.length, rules = scope.parser.rules) {
    if (start >= end) return;
    ...
    if (this.excludes) {
      ...
      // clone the rule
      rule = new Rule.Group(rule);
      rule.rules = rule.rules.filter(rule => !this.excludes.includes(rule.name));
      rules = {
        ...rules,
        [this.subrule]: rule
      }
    ...
    const match = rule.parse(scope, tokens, start, end, rules);
...
Rule.Choice = class choices extends Rule {
  ...
  test(scope, tokens, start, end, rules, testLocation = this.testLocation) {
    let undefinedFound = false;
    for (let i = 0, rule; rule = this.rules[i]; i++) {
      const result = rule.test(scope, tokens, start, end, rules, testLocation);
  ...
  parse(scope, tokens, start = 0, end = tokens.length, rules = scope.parser.rules) {
    ...
    const matches = [];
    for (let i = 0, rule; rule = this.rules[i]; i++) {
      let match = rule.parse(scope, tokens, start, end, rules);
...
Rule.Repeat = class repeat extends Rule {
  ...
  test(scope, tokens, start, end, rules, testLocation = this.testLocation) {
    if (this.testRule) return this.testRule.test(scope, tokens, start, end, rules, testLocation);
  }

  parse(scope, tokens, start = 0, end = tokens.length, rules) {
    if (start >= end) return;
    // Bail quickly if no chance
    if (this.test(scope, tokens, start, end, rules) === false) return undefined;

    const matched = [];
    let nextStart = start;
    while (nextStart < end) {
      let match = this.repeat.parse(scope, tokens, nextStart, end, rules);
      if (!match) break;
...
Rule.List = class list extends Rule {
  ...
  parse(scope, tokens, start = 0, end = tokens.length, rules = scope.parser.rules) {
    ...
    while (nextStart < end) {
      ...
      let item = this.item.parse(scope, tokens, nextStart, end, rules);
      if (!item) break;
      ...
      let delimiter = this.delimiter.parse(scope, tokens, nextStart, end, rules);
      if (!delimiter) break;
...
Rule.Sequence = class sequence extends Rule {
  ...
  parse(scope, tokens, start = 0, end = tokens.length, rules = scope.parser.rules) {
    ...
    for (let i = 0, rule; rule = this.rules[i]; i++) {
      ...
      let match = rule.parse(scope, tokens, nextStart, end, rules);
  ...
  parse(scope, tokens, start = 0, end = tokens.length, rules = scope.parser.rules) {
    ...
    if ((tokens[end - 1] instanceof Token.Comment)) {
      comment = Rule.getRuleOrDie(rules, "comment")
        .parse(scope, tokens, end - 1, end, rules);
    }

    const match = super.parse(scope, tokens, start, end, rules);
...
Rule.Statements = class statements extends Rule {
  ...
  parseBlock(scope, block, start) {
    let matched = [];
    const statementRule = Rule.getRuleOrDie(scope.parser.rules, "statement");
