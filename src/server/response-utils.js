...
export async function sendTextFile(response, path, defaultValue) {
  ...
  if (defaultValue !== undefined)
    return response.send(defaultValue);
...
export function sendJSONFiles(response, paths, optional = false) {
  return fileUtils.loadFiles(paths, "utf8", optional)
    .then(jsonBlocks => {
      ...
      jsonBlocks.forEach( (json, index) => {
        ...
        if (json === null)
          console.warn(chalk.grey("  " + paths[index]));
        else
          console.warn("  " + paths[index]);
      });
  ...
  return fileUtils.loadFiles(paths, "utf8", optional)
    .then(jsonBlocks => {
      ...
      jsonBlocks.forEach( (json, index) => {
        ...
        // Log missing / empty files in lighter color
        if (json === null)
          return console.warn(chalk.grey(message));
...
export function sendError(response, statusCode, error, errorMessage = error && error.message) {
  if (!statusCode || !error)
    throw new TypeError("sendError() requires both statusCode and error params");
