...
//
//  Parse a rule with `rule.parse(parser, tokens, start, end, stack, rules, rules)`, this will either:
...
Rule.Alternatives = class alternatives extends Rule {
  ...
  parse(parser, tokens, start = 0, end = tokens.length, stack, rules = parser.rules) {
    ...
    const matches = [];
    for (let i = 0, rule; rule = this.rules[i]; i++) {
      let match = rule.parse(parser, tokens, start, end, stack, rules);
...
Rule.Repeat = class repeat extends Rule {
  ...
  parse(parser, tokens, start = 0, end = tokens.length, stack, rules = parser.rules) {
    ...
    let nextStart = start;
    while (nextStart < end) {
      let match = this.repeat.parse(parser, tokens, nextStart, end, stack, rules);
      if (!match) break;
...
Rule.List = class list extends Rule {
  ...
  parse(parser, tokens, start = 0, end = tokens.length, stack, rules = parser.rules) {
    ...
    while (nextStart < end) {
      ...
      let item = this.item.parse(parser, tokens, nextStart, end, stack, rules);
      if (!item) break;
      ...
      let delimiter = this.delimiter.parse(parser, tokens, nextStart, end, stack, rules);
      if (!delimiter) break;
