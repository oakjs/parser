...
//
//  Parse a rule with `rule.parse(parser, tokens, start, end, rules)`, this will either:
...
Rule.Subrule = class subrule extends Rule {
  ...
  parse(parser, tokens, start = 0, end = tokens.length, rules = parser.rules) {
    const rule = this.getRuleOrDie(rules, this.subrule);
    const match = rule.parse(parser, tokens, start, end, rules);
    if (!match) return undefined;
...
Rule.Repeat = class repeat extends Rule {
  ...
  parse(parser, tokens, start = 0, end = tokens.length, rules) {
    ...
    let nextStart = start;
    while (nextStart < end) {
      let match = this.repeat.parse(parser, tokens, nextStart, end, rules);
      if (!match) break;
...
Rule.List = class list extends Rule {
  ...
  parse(parser, tokens, start = 0, end = tokens.length, rules = parser.rules) {
    ...
    while (nextStart < end) {
      ...
      let item = this.item.parse(parser, tokens, nextStart, end, rules);
      if (!item) break;
      ...
      let delimiter = this.delimiter.parse(parser, tokens, nextStart, end, rules);
      if (!delimiter) break;
...
Rule.Statement = class statement extends Rule.Group {
  parse(parser, tokens, start = 0, end = tokens.length, rules = parser.rules) {
    ...
    const match = super.parse(parser, tokens, start, end, rules);
