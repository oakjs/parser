...
Rule.breakIntoBlocks = function (tokens) {
	...
	var lines = Rule.breakIntoLines(tokens);
	if (lines.length === 0) return [];

	// figure out indents
	var indents = Rule.getLineIndents(lines);

	// First block is at the MINIMUM indent of all lines!
	var maxIndent = Math.min.apply(Math, indents);
	var block = new Rule.Block({ indent: maxIndent });
...
Rule.getLineIndents = function (lines) {
	...
	_createClass(statements, [{
		...
		value: function parseStatement(parser, tokens, indent) {
			...
			if (!statement && !comment) {
				var unparsed = tokens.slice(start, end).join(" ");
				var error = new Rule.StatementParseError({ unparsed: unparsed, indent: indent });
				results.push(error);
			}

			// complain can't parse the entire line!
			else if (statement && statement.nextStart !== end) {
					var parsed = tokens.slice(start, statement.nextStart).join(" ");
					var _unparsed = tokens.slice(statement.nextStart, end).join(" ");
					var _error = new Rule.StatementParseError({ parsed: parsed, unparsed: _unparsed, indent: indent });
					results.push(_error);
				}
...
var Tokenizer = {
	...
	findNextNewline: function findNextNewline(tokens) {
		var start = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 0;
		var end = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : tokens.length;
	...
	getNextNonEmptyLineIndent: function getNextNonEmptyLineIndent(lines) {
		var start = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 0;
		var end = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : lines.length;
